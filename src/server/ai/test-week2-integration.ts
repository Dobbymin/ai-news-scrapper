...
import { loadAnalysis, loadMarketData, loadNews } from "../storage/json-store.server";
...
async function step3AnalyzeAndCalculate(): Promise<boolean> {
  ...
  try {
    ...
    const onProgress = (progress: AnalysisProgress) => {
      console.log(`⏳ 분석 진행: ${progress.current}/${progress.total} (${progress.percentage}%)`);
    };

    // 분석 실행
    const { result, time } = await measureTime(() => analyzeAndCalculate(new Date(), onProgress));
