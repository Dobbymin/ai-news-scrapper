...
import { scrapeNaverNews, removeDuplicateNews } from './scraper-naver-news.server';
import { saveNews } from '../storage/json-store.server';
import { 
  scrapeAllMarketData, 
  collectAndSaveMarketData,
  printMarketSummary 
} from './scraper-market.server';
...
async function runIntegratedTest() {
  ...
  // 성능 분석
  console.log('\n📊 성능 분석:');
  console.log(`   - 뉴스 크롤링 속도: ${newsResult.success && newsResult.dataCount ? (newsResult.dataCount / (newsResult.duration / 1000)).toFixed(2) : 0}개/초`);
