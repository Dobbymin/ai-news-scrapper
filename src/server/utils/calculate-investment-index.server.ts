...
export function calculateInvestmentIndex(analyses: SentimentAnalysis[]): number {
  ...
  for (const analysis of analyses) {
    // 신뢰도에 따른 가중치
    const weight =
      analysis.confidence >= HIGH_CONFIDENCE_THRESHOLD
 ...
 * @returns 투자 지수 (0-100, 소수점 첫째자리)
 */
export function calculateSimpleInvestmentIndex(
 ...
 * @returns 감성별 평균 신뢰도
 */
export function calculateAverageConfidenceBySentiment(
  analyses: SentimentAnalysis[]
): {
  positive: number;
  negative: number;
  ...
  const avgPositive =
    positiveAnalyses.length > 0
      ? positiveAnalyses.reduce((sum, a) => sum + a.confidence, 0) /
        positiveAnalyses.length
      : 0;
  
  const avgNegative =
    negativeAnalyses.length > 0
      ? negativeAnalyses.reduce((sum, a) => sum + a.confidence, 0) /
        negativeAnalyses.length
      : 0;
  
  const avgNeutral =
    neutralAnalyses.length > 0
      ? neutralAnalyses.reduce((sum, a) => sum + a.confidence, 0) /
        neutralAnalyses.length
      : 0;
