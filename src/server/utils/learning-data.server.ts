...
export function extractSuccessCases(
  ...
  return accuracyLogs
    ...
    .map((log) => {
      ...
      // 평균 신뢰도 계산
      const avgConfidence =
        analysis.newsAnalysis.reduce((sum, a) => sum + a.confidence, 0) /
        analysis.newsAnalysis.length;
...
export function createLearningData(
  ...
  // 전체 평균 정확도
  const avgAccuracy =
    accuracyLogs.reduce((sum, log) => sum + log.accuracy, 0) / accuracyLogs.length;
  
  // 방향 정확도
  const directionAccuracy =
    (accuracyLogs.filter((log) => log.isCorrect).length / accuracyLogs.length) * 100;
